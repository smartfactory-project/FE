<template>
  ...
  <div class="paint-summary">
    <h3 class="summary-title">PAINT 종합 성과지표</h3>
...
// paint 하드코딩 데이터
const paints = [
  { name: 'paint1', production: 305, working: 85, failed: 2 },
  { name: 'paint2', production: 302, working: 85, failed: 2 },
  { name: 'paint3', production: 198, working: 70, failed: 12 },
  { name: 'paint4', production: 195, working: 92, failed: 1 },
  { name: 'paint5', production: 296, working: 75, failed: 2 },
];

const totalProduction = computed(() => paints.reduce((sum, p) => sum + p.production, 0));
// paint 종합 성과지표 하드코딩
const avgEfficiency = 72;
const avgOperationRate = 92.7;
const avgQualityRate = 90.4;
...
const weeklyChartData = ref({
  ...
  datasets: [
    {
      label: 'PAINT 주간 생산량',
      data: paints.map(p => p.production),
...
// 품질 분포(원형)
const totalWorking = paints.reduce((sum, p) => sum + p.working, 0);
const totalFailed = paints.reduce((sum, p) => sum + p.failed, 0);
...
const qualityRateChartData = ref({
  ...
  datasets: [
    {
      label: '품질률(%)',
      data: paints.map(p => ((p.working/(p.working+p.failed))*100).toFixed(2)),
...
const goalAchievementChartData = ref({
  ...
  datasets: [
    {
      label: 'PAINT 목표 달성율(%)',
      data: paints.map(p => ((p.production/350)*100).toFixed(1)),
      borderColor: '#34d399',
...
const operationRateChartData = ref({
  ...
  datasets: [
    {
      label: '가동률(%)',
      data: paints.map(p => ((p.working/(p.working+p.failed))*100).toFixed(1)),
