  ...
  <div class="min-h-screen bg-gradient-to-br from-slate-50 via-blue-50 to-indigo-100">
    ...
    <main class="container mx-auto px-6 py-8">
      ...
      <div class="flex items-center gap-3 mb-8">
        ...
        <div>
          <h1 class="text-3xl font-bold text-foreground">전체 공지사항</h1>
          <p class="text-muted-foreground mt-1">SAMJO의 모든 공지사항을 확인하세요</p>
        </div>
      ...
      <div class="grid gap-6">
        <Card
          ...
          :key="a.id"
          class="group hover:shadow-lg transition-all duration-300 border-0
                 bg-gradient-to-r from-white to-slate-50 hover:from-white hover:to-white backdrop-blur-sm"
        >
          <CardContent class="p-6">
            <div class="flex items-start gap-4">
              <div class="text-3xl">{{ getTypeIcon(a.type) }}</div>

              <div class="flex-1 min-w-0">
                <div class="flex items-center gap-3 mb-3">
                  <Pin v-if="a.isPinned" class="h-4 w-4 text-indigo-600 fill-current" />
                  <h2 class="font-bold text-xl text-foreground group-hover:text-indigo-600 transition-colors">
                    {{ a.title }}
                  </h2>
                  <Badge variant="outline" :class="['ml-auto', getTypeColor(a.type)]">
...
import { Bell, Pin, Calendar, Users, ArrowLeft } from 'lucide-vue-next'
